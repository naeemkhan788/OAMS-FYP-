...
const seedData = async () => {
  try {
    ...
    console.log('Student credentials -> email: student1@oams.local, password: Student123\n');
    
    // Keep in-memory server running
    console.log('✅ In-memory MongoDB is running and seeded!');
    console.log('🚀 Now run: npm start (in backend folder)');
    console.log('   Frontend will connect to localhost:5000\n');
    
    // Don't exit - keep server alive
    if (process.env.KEEP_ALIVE !== 'false') {
      console.log('Keeping in-memory MongoDB running... (Ctrl+C to stop)');
    } else {
